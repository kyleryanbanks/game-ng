...
export const connected$ = fromEvent<GamepadEvent>(window, 'gamepadconnected');
export const disconnected$ = fromEvent<GamepadEvent>(
  window,
  'gamepaddisconnected'
);
...
) => Observable<IFrameData> = (prevFrame: IFrameData) => {
...
export const getButtonsPerFrames = () => {
  return connected$.pipe(
    tap(console.log),
    map((event) => event.gamepad.index),
    switchMap((id: number) => {
      return getFrames().pipe(
        map(() => {
...
export const getMappedButtons = () => {
  return getButtonsPerFrames().pipe(
    map((buttons) => {
      return {
        TAG: buttons['2'],
        LP: buttons['3'],
...
export const getDirection = (): Observable<number | null> => {
  return getButtonsPerFrames().pipe(
    map((buttons) => {
...
export const getInputs = () => {
  return combineLatest([getDirection(), getMappedButtons()]).pipe(
